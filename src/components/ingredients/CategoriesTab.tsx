...
export const CategoriesTab = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    let query = supabase.from('ingredient_categories').select('*').order('created_at', { ascending: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingId) {
      await supabase.from('ingredient_categories').update(formData).eq('id', editingId);
    } else {
      await supabase.from('ingredient_categories').insert([formData]);
    }
  ...
  return (
    <div className="py-2">
        ...
        <table className="table table-hover align-middle mb-0" style={{ fontSize: '13px' }}>
          ...
          <tbody className="border-top-0 bg-white">
            {loading ? (
              ...
              categories.map((c) => (
                <tr key={c.id}>
                  <td className="px-4 py-3 fw-bold text-dark">{c.name}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Sửa Danh Mục' : 'Thêm Danh Mục'}>
